...
// ponytail: added logoText + item.onClick support; rest is upstream source
export const StaggeredMenu = ({
  position = 'right',
  colors = ['#d4d4d4', '#111111'],
  items = [],
  socialItems = [],
  displaySocials = true,
  ...
  const animateColor = useCallback(opening => {
    ...
    if (!btn || !changeMenuColorOnOpen) { if (btn) gsap.set(btn, { color: menuButtonColor }); return; }
  ...
  // ponytail: derive pre-layers (trim middle layer if ≥3)
  const preLayers = (() => {
    const raw = colors && colors.length ? colors.slice(0, 4) : ['#e0e0e0', '#111'];
    const arr = [...raw];
  ...
  return (
    ...
    >
      ...
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {preLayers.map((c, i) => <div key={i} className="sm-prelayer" style={{ background: c }} />)}
      </div>
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <div className="sm-logo">
          {logoUrl
            ...
            : logoText
              ? <span className="sm-logo-text">{logoText}<span className="sm-logo-dot" /></span>
              : null
          }
        </div>
      ...
      <aside id="staggered-menu-panel" ref={panelRef} className="staggered-menu-panel" aria-hidden={!open}>
        {/* Batik decorative background (bottom gradient, B&W) */}
        <div className="sm-batik-bg" aria-hidden="true" />
